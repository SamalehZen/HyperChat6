...
export async function GET(request: NextRequest, { params }: { params: { mode: string } }) {
  ...
  const modeKey = decodeURIComponent(params.mode || '');
  ...
  const rows = await prisma.messageLog.findMany({
    where: { mode: modeKey, createdAt: range },
  ...
  for (const b of bins) {
    const [cCount, eCount, aCount] = await Promise.all([
      prisma.messageLog.count({ where: { mode: modeKey, createdAt: { gte: b.start, lte: b.end }, status: 'COMPLETED' } }),
      prisma.messageLog.count({ where: { mode: modeKey, createdAt: { gte: b.start, lte: b.end }, status: 'ERROR' } }),
      prisma.messageLog.count({ where: { mode: modeKey, createdAt: { gte: b.start, lte: b.end }, status: 'ABORTED' } }),
    ]);
  ...
  // Correlation latence × statut (tranches)
  const corrBuckets = edges.map((start, i) => {
    const end = edges[i+1] ?? Infinity;
    const label = isFinite(end) ? `${start}-${end} ms` : `${start}+ ms`;
    const inBucket = rows.filter(r => r.latencyMs >= start && (isFinite(end) ? r.latencyMs < end : true));
    return {
      label,
      statusCounts: {
        COMPLETED: inBucket.filter(r => r.status === 'COMPLETED').length,
        ERROR: inBucket.filter(r => r.status === 'ERROR').length,
        ABORTED: inBucket.filter(r => r.status === 'ABORTED').length,
      },
    };
  });
  ...
  return NextResponse.json({
    ...
    correlation: { buckets: corrBuckets },
